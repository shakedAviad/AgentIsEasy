    ...
    public static async Task RunAsync(string openAiApiKey, string gitHubPat)
    {
        ...
        var agent = factory.CreateAgent(new OpenAIOptions
        {
            ...
            Instructions = """
                ...
                """,
            Tools = allTools,
            LoggingMiddleware = new LoggingMiddleware(LoggerFactory.Create(builder => builder.AddConsole()), (loggingAgent) => { }),
            RawToolCallDetails = (details) => Console.WriteLine(details)
        });
